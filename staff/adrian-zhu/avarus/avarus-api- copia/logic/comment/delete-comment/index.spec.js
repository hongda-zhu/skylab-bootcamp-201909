...
describe('logic - delete comment', () => {
    ...
    let body = 'this is a comment'
        ...
        it('should create successfully a comment with correct information', async () => {
            ...
            expect(deleteCommentObject).not.to.exist
        ...
        it('should failed with wrong comment id', async () => {
            ...
            try {
                await deleteComment(wrongCommentId)
    ...
    it('should fail on incorrect userId, companyId, transactionId or expression type and content', () => {
        ...
        expect(() => deleteComment(1)).to.throw(TypeError, '1 is not a string')
        expect(() => deleteComment(true)).to.throw(TypeError, 'true is not a string')
        expect(() => deleteComment([])).to.throw(TypeError, ' is not a string')
        expect(() => deleteComment({})).to.throw(TypeError, '[object Object] is not a string')
        expect(() => deleteComment(undefined)).to.throw(TypeError, 'undefined is not a string')
        expect(() => deleteComment(null)).to.throw(TypeError, 'null is not a string')
        expect(() => deleteComment('')).to.throw(ContentError, 'commentId is empty or blank')
        expect(() => deleteComment(' \t\r')).to.throw(ContentError, 'commentId is empty or blank')
